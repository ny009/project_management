          ...
          <CCardBody>
            <CTabs >
              ...
              <CTab :title="p.name" active>
                ...
                <div>
                  <h4>Submit {{p.name}}</h4>
                  <CForm :model = "submitfile">
                    <CRow>
                      <CCol>
                          <CInput type="file" v-model="submitfile.filename"/>
                      </CCol>
                      <CCol>
                          <CButton type="submit" color="primary" @click="submitphase(p.id)">submit</CButton>
                      </CCol>
                    </CRow>
                  </CForm>
                </div>
...
export default {
  ...
  methods:{
    ...
    submitphasse:function(phaseid,filename){
      var self = this;
      axios.post("http://127.0.0.1:5000/team/invite",{
        team_id:self.task.team_id,
        phase_id:phaseid,
        file:self.submitfile.filename
      },{emulateJSON:true})
      .then(function(response){
        if(response.data.status === 'SUCCESS'){
